...
sys.path.insert(0, os.path.join(THIS_SCRIPT_DIR, "OpticsCheck"))
sys.path.insert(0, os.path.join(THIS_SCRIPT_DIR, "SVBeamMeasure"))
sys.path.insert(0, os.path.join(THIS_SCRIPT_DIR, "SVCalibration"))

# from esper_explore_widgets 
from esper_explore_widgets import rebus_napari_menu, fov_utils, napari_file_explore_widget
# import rebus_napari_menu, fov_utils
# import napari_file_explore_widget
from SVBeamMeasure import ArgoDataLib
from SVBeamMeasure import GaussFitLib


NUM_CHANNELS = 4
Metadata_Text_Columns = \
    ["System serial #", "System model #", "Acquisition start time", "Stage position (XY)", "Stage position (Z)", \
        "Excitation (nm) - Emission (nm)", "Laser power", "Camera exposure time per raw image frame", \
            "Number of raw image frames per FOV", "Camera sensor resolution"]


@magicgui(
    auto_call=False, 
    # slide={"choices": list(all_slides)},
    # layout='horizontal',
    layout='vertical',
    call_button="Generate SVBeamMeasure",        
    )
def SVBeamMeasure(viewer : napari.Viewer):
    ...
    try:
        ...
        for layer in all_layers:
            if layer._type_string=='image':
                ...
                plt.imsave(os.path.join(save_dir_path, fnameTitle_combined), imgs_comb)
...
def main():
    ...
    rebus_menu = rebus_napari_menu.RebusMenu(viewer, viewer.window)
    viewer.window.main_menu.addMenu(rebus_menu)
    
    FolderBrowser_widget = napari_file_explore_widget.FolderBrowser(viewer)
    viewer.window.add_dock_widget(FolderBrowser_widget, name='Load Experiment Files', area='right', menu=viewer.window.window_menu)
    viewer.window.add_dock_widget(SVBeamMeasure, name='SVBeamMeasure', area='right', menu=viewer.window.window_menu)    
